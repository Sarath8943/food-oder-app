...
const addReview = async (req, res) => {
  try {
    ...
    if (!order || order.status !== "Delivered") {
      return res.status(404).json({ message: "Order not delivered or not found" });
    }

    const isMenuInOrder = order.cartId.items.some(
      (item) => item.menuId.toString() === menuId.toString()
    );
    if (!isMenuInOrder) {
      return res.status(400).json({ message: "Menu item not part of the order." });
    }

    const existingReview = await Review.findOne({ userId, menuId, orderId });
    if (existingReview) {
